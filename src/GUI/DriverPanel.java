...
public class DriverPanel extends JPanel {
    ...
    private JPanel createTrajetsView() {
        ...
        // Table - Updated with Date/Time columns
        String[] columns = {"Départ", "Arrivée", "Départ (Date/Heure)", "Arrivée (Date/Heure)", "Durée", "Prix (TND)", "Statut", "Passager"};
    ...
    private JPanel createNewTrajetView() {
        ...
        JLabel startDateTimeLabel = StyleUtils.createLabel("📅 Date et Heure de Départ :");
        startDateTimeLabel.setFont(new Font("Segoe UI", Font.BOLD, 12));
        formCard.add(startDateTimeLabel, gbc);
        
        // Start Date/Time Picker
        gbc.gridx = 0; gbc.gridy = 5; gbc.gridwidth = 2;
        DateTimePickerPanel startDateTimePicker = new DateTimePickerPanel(java.time.LocalDateTime.now().plusHours(1));
        formCard.add(startDateTimePicker, gbc);
        
        // End Date/Time Label
        gbc.gridx = 0; gbc.gridy = 6; gbc.gridwidth = 2;
        JLabel endDateTimeLabel = StyleUtils.createLabel("📅 Date et Heure d'Arrivée :");
        endDateTimeLabel.setFont(new Font("Segoe UI", Font.BOLD, 12));
        formCard.add(endDateTimeLabel, gbc);
        
        // End Date/Time Picker
        gbc.gridx = 0; gbc.gridy = 7; gbc.gridwidth = 2;
        DateTimePickerPanel endDateTimePicker = new DateTimePickerPanel(java.time.LocalDateTime.now().plusHours(2));
        formCard.add(endDateTimePicker, gbc);
        ...
        createBtn.addActionListener(e -> {
            ...
            java.time.LocalDateTime startDateTime = startDateTimePicker.getSelectedDateTime();
            java.time.LocalDateTime endDateTime = endDateTimePicker.getSelectedDateTime();
            ...
            if (!validateDateTimes(startDateTime, endDateTime)) {
                return;
            }
            
            createTrajet(depart, arrivee, duree, (float) prix, startDateTime, endDateTime);
            departField.setText("");
            arriveeField.setText("");
            dureeSpinner.setValue(30);
            prixSpinner.setValue(5.0);
            // Reset date/time pickers
            startDateTimePicker.setSelectedDateTime(java.time.LocalDateTime.now().plusHours(1));
            endDateTimePicker.setSelectedDateTime(java.time.LocalDateTime.now().plusHours(2));
        });
    ...
    /**
     * Validates that start date/time is before end date/time and both are in the future
     */
    private boolean validateDateTimes(java.time.LocalDateTime startDateTime, java.time.LocalDateTime endDateTime) {
        if (startDateTime == null || endDateTime == null) {
            StyleUtils.showError(this, "Veuillez sélectionner une date et une heure valides !");
            return false;
        }
        ...
        if (startDateTime.isBefore(now)) {
            StyleUtils.showError(this, "La date/heure de départ ne peut pas être dans le passé !");
            return false;
        }
        
        // Check if end time is before or equal to start time
        if (endDateTime.isBefore(startDateTime) || endDateTime.equals(startDateTime)) {
            StyleUtils.showError(this, "La date/heure d'arrivée doit être après la date/heure de départ !");
            return false;
        }
    ...
    private void createTrajet(String depart, String arrivee, int dureeMinutes, float prix, 
        ...
        try {
            ...
            // Use the new constructor with date/time
            ...
            StyleUtils.showSuccess(this, "Trajet créé avec succès !\n" + 
                depart + " → " + arrivee + "\n" +
                "Départ: " + startDateTime.format(formatter) + "\n" +
                "Arrivée: " + endDateTime.format(formatter) + "\n" +
                "Prix: " + prix + " TND");
    ...
    private void refreshTrajetsTable() {
        ...
        for (Trajet t : mainFrame.getGestion().getTrajets()) {
            if (t.getConducteur() != null && 
                ...
                String startDateTimeStr = "N/A";
                String endDateTimeStr = "N/A";
                if (t.getStartDateTime() != null) {
                    startDateTimeStr = t.getStartDateTime().format(formatter);
                }
                if (t.getEndDateTime() != null) {
                    endDateTimeStr = t.getEndDateTime().format(formatter);
                }
    ...
    /**
     * Supprimer un passager accepté
     * Demande une confirmation et effectue la suppression avec notification
     */
    private void deletePassengerFromAccepted() {
        int selectedRow = passagersAcceptesTable.getSelectedRow();
        if (selectedRow == -1) {
            StyleUtils.showError(this, "Veuillez sélectionner un passager à supprimer");
            return;
        }
                    ...
                    if (count == selectedRow) {
                        ...
                        if (confirm == JOptionPane.YES_OPTION) {
                            ...
                            if (deleted) {
                                ...
                                StyleUtils.showError(this, "Erreur lors de la suppression du passager.");
                            }
                        }
                        return;
                    }
        ...
        StyleUtils.showError(this, "Erreur: Passager non trouvé");
    }
}
